...
BogusTargetLowering::BogusTargetLowering(const TargetMachine &TM,
                                         const BogusSubtarget &STI)
    : TargetLowering(TM), Subtarget(STI)
{
  // Set up the register classes
  addRegisterClass(MVT::i32, &Bogus::GPRRegClass);
...
const char *BogusTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case BogusISD::Ret: return "BogusISD::Ret";
  default:            return NULL;
  }
}

void BogusTargetLowering::ReplaceNodeResults(SDNode *N,
...
static const MCPhysReg GPRArgRegs[] = {
  Bogus::X0, Bogus::X1, Bogus::X2, Bogus::X3
};

/// LowerFormalArguments - transform physical registers into virtual registers
/// and generate load operations for arguments places on the stack.
SDValue BogusTargetLowering::LowerFormalArguments(
                                    SDValue Chain,
                                    ...
                                    SmallVectorImpl<SDValue> &InVals) const
{
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, Bogus_CCallingConv);
...
SDValue
BogusTargetLowering::LowerReturn(SDValue Chain,
                                 CallingConv::ID CallConv, bool isVarArg,
                                 const SmallVectorImpl<ISD::OutputArg> &Outs,
  ...
  CCInfo.AnalyzeReturn(Outs, Bogus_CRetConv);
  ...
  return DAG.getNode(BogusISD::Ret, dl, MVT::Other, RetOps);
}
...
SDValue
BogusTargetLowering::LowerRETURNADDR(SDValue Op, SelectionDAG &DAG) const {
  return SDValue();
}
